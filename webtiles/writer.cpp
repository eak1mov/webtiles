...
Zoom calcMaxZoom(const index::IndexMap& indexMap)
{
    WT_ASSERT(!indexMap.empty());
    return std::ranges::max_element(indexMap, {}, [](const auto& kv) { return kv.first.z; })
        ->first.z;
}
    ...
    index::IndexMap indexMap)
{
    ...
    if (writerParams.blockLevels) {
        maxZoom = writerParams.blockLevels->back() - 1;
        blockLevels = *writerParams.blockLevels;
        WT_ASSERT(writerParams.indexFormat != IndexFormat::BasicPlain);
        WT_ASSERT(indexMap.empty() || calcMaxZoom(indexMap) <= maxZoom);
    } else if (indexMap.empty()) {
        maxZoom = MAX_ZOOM;
        blockLevels = {};
    } else if (writerParams.indexFormat == IndexFormat::BasicPlain) {
        maxZoom = calcMaxZoom(indexMap);
